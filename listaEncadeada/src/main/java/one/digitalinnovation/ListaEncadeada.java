...
public class ListaEncadeada<T> {
    ...
    public int size(){
        ...
        while (true){
            if (referenciaAuxilar != null){
                tamanhoLista++;
                if (referenciaAuxilar.getProximoNo() != null){
    ...
    public String toString() {
        String strRetorno = "";
        No<T> noAuxilair = referenciaEntrada;
        for (int i = 0; i < this.size(); i++){
            strRetorno += "[No{conteúdo = " + noAuxilair.getConteudo() + "}]--->";
            noAuxilair = noAuxilair.getProximoNo();
        }
        strRetorno += "null";
